...
begin
 ...
 then
  ...
  begin
   ...
   if (nick.Find(n,i3)<>true)
    and (n<>form1.Edit2.Text)
    and (bol<>true)
   then nick.Add(n);
   Bol:=false;
  end;
 ...
 then
  ...
  with IdFTP1 do
   try
    ...
    StatusBar1.Refresh;
// !!!            ИЗМЕНИТЬ на СВОИ ДАННЫЕ                  !!!
    Username:='username'; // изменить
    Password:='paswd';
    Host:='ftp.yoursite.ru';
    Connect;
    ListView1.Clear;
    Statusbar1.Panels.Items[1].Text:=' вход в каталог';
    StatusBar1.Refresh;
    if f.ReadBool('Options','Chat',true)=true
    then IdFTP1.ChangeDir('/upload/1. Soft от H.K.D/Soft/data')
...
begin
 ...
 if ob=true
 then Showmessage('Идёт обновление...');
 Form1.Caption:=FormCaption;
 sl:=TstringList.Create;
 if Edit2.Text='Логин'
 then Showmessage('Введите СВОЙ логин')
 else if Edit2.Text=''
 then Showmessage('Зарегистрируйте свой ник')
 else if Edit1.Text=''
 then Showmessage('Прежде чем добавить нужно ввести сообщение!')
 else
  begin
   if (Edit3.Text='Всем') or (Edit3.Text='')
   then nic:=''
   else nic:=Edit3.Text+', ';
   if timetostr(Time)[2]=':'
   then tt:=copy(timetostr(Time),0,4)+' '
   ...
   then
    ...
    begin
     ...
     RichEdit2.Lines.Assign(sl);
     Re_SL(sl,false);
     ProgressBar1.Position:=30; StatusBar1.Refresh;
 ...
 then
  begin
   ...
   then
     ...
     then
      begin
       for j:=0 to s.Count-2 do
        begin
         if (s.Strings[j]=s2.Strings[s2.Count-1])
         then
          begin
           if f.ReadBool('Sound','New',true)=true
           then PlaySound(PChar('SYSTEMASTERISK'), 0, SND_ASYNC);
           Form1.Caption:=FormCaption+' Появились новые сообщения: '+inttostr(s.Count-j-1);
           break;
          end
         else Form1.Caption:=FormCaption;
        end;
      end;
    end;
    Get_Nic(s,nick);
    ...
    for j:=0 to nick.Count-1 do
     begin
      if (nick.Strings[j]<>'') and (nick.Strings[j]<>' ')
      then
       begin
   ...
   for j:=0 to Priva.Count-1 do
    begin
    if (Priva.Strings[j]=priv) and (Priva.Strings[j]<>'')
    then PR:=j;
    end;
   if PR<>100
   then Combobox2.ItemIndex:=PR;
   Re_SL(s,true);
...
begin
 ...
 if f.ReadBool('Options','Color',false)=false
 then
  begin
   col[0]:=clBlack;
   col[1]:=clBlack;
 ...
 if f.ReadBool('Options','Color',false)=true
 then RichEdit1.Color:=clBlack
 else
  begin
   if f.ReadBool('Options','FontColor',true)=true
   then
    begin
     col[0]:=clBlack;
     col[1]:=clBlack;
 ...
 if f.ReadString('Other','Name','Логин')='Логин'
 then
  begin
 ...
 if f.ReadBool('Options','Admin',false)=true
 then Button9.Visible:=true;
 f.Free;
...
begin
 ...
 Button2.Left:=round(500-x);
 Button2.Top:=round(353-y);
 Edit1.Top:=round(353-y);
 Edit3.Top:=round(353-y);
 RichEdit1.Width:=round(497-x);
 RichEdit2.Width:=round(497-x);
 RichEdit1.Height:=round(295-y);
 Listview1.Left:=round(500-x);
 ListView1.Height:=round(295-y);
 Combobox2.Top:=round(353-y);
 Image1.Width:=Form1.Width;
 Image1.Height:=round(401-y);
 Button2.Width:=Listview1.Width;
 Edit1.Width:=RichEdit1.Width+RichEdit1.Left-Edit3.Width-ComboBox2.Width-32;
end;
...
{-------------------Смайлы-----------------}
 ...
 if Get_Priv(Lines[Arow-1])=Form1.Edit2.Text+','
 then
  begin
...
begin
 ...
 for i:=0 to RE.Lines.Count do
  begin
   ...
   if length(s)>7
   then s2:=s[8];
   if (s2='0') or (s2='1') or (s2='2') or (s2='3') or (s2='4') or (s2='5') or (s2='6') or (s2='7') or (s2='8') or (s2='9')
   then j:=strtoint(s2)
   else
    if Length(s)>1
    then
